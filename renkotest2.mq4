      ...
      if(Bid >= base + brick_size*Point)
        {
         ...
         ObjectCreate(time_str, OBJ_RECTANGLE, 0, t0-1,base,Time[0],base + brick_size*Point);
      ...
      if(Ask <= base - brick_size*Point)
        {
         ...
         ObjectCreate(time_str, OBJ_RECTANGLE, 0, t0-1, base - brick_size*Point,Time[0],base - brick_size*Point);
   ...
   if(direction == 1)
     {
      ...
      int high_candle = iHighest(Symbol(), Period(),MODE_HIGH,interval/60);
      if(ObjectCreate)
        {
      ...
      if(Ask <= base - 2*brick_size*Point)
        {
         ...
         ObjectCreate(time_str, OBJ_RECTANGLE, 0, t0-1, base - brick_size*Point,Time[0],base - 2*brick_size*Point);
         ObjectSet(time_str,OBJPROP_COLOR,clrRed);
   ...
   if(direction == -1)
     {
      ...
      if(Bid >= base + 2*brick_size*Point)

        {
         ...
         //ObjectCreate(time_str + "1",OBJ_,0,mp, Open[0],mp,High);//how do we get highest over bar range?
